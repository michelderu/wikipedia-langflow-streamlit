...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Real-time updates", "Political news", "Celebrities", "Stock", "Search the world"])
...
# Initialize the stream dataframe
...
def show_chat_qa(question, date, answer_placeholder, sources_placeholder):
    ...
    results = collection.find(
        ...
        limit=10,
        projection={"title", "content", "date", "url"}, # only return these fields from the document
    ...
    for result in results:
        ...
        sources += f"**[{result['title']}]({result['url']})**&nbsp;&nbsp;&nbsp;📅&nbsp;&nbsp;{result['date']}&nbsp;&nbsp;&nbsp;📈&nbsp;{round(result['$similarity'] * 100, 1)}%\\\n"
...
def show_search_results(results, placeholder):
    ...
    with placeholder:
        for result in results:
            metadata = get_metadata(result['title'], result['content'])
...
with tab5:
    ...
    date_toggle = col1.toggle("Filter by date", value=True)
